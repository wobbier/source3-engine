...
class hctFilterManagerInterface
{
	public:
		...
		virtual void removeAllStoredObjects() = 0;
			...
			/// Returns the Dll manager
		virtual const class hctFilterDllManager* getDllManager() const = 0;

			/// Returns the path where the Havok content tools were installed.
		virtual const char* getHavokPath() const = 0;
};
